...
# This script restores a file/directory trashed from the current directory
...
done | sort -r | fzf -i -e +s -m | sed -E 's/.* [D ] (.* \| .*)/\1/' |

    while read line

    do
	# select $basename from selection
	basename_file=$(echo $line | sed -E 's/.* \| (.*)/\1/')

	original_path_file=$(echo $line | sed -E 's/(.*) \| .*/\1/')
