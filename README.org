...
- while trash-empty from [[https://github.com/andreafrancia/trash-cli][trash-cli]] empties all the trashes from all mount points (except for the root trash which requires ~sudo trash-empty~), tra.sh will empty only the trash of the current mount point. This prevents the accidental deletion of files in the trash of an external drive or the deletion of files in the user's trash when cleaning an external drive's trash
